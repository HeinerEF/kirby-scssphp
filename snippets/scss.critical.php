...
if(isset($scss) AND !($scss === '')) {
  ...
  if($Echo4Developer) {
    echo '<!-- scss file "' . $template . '.critical.scss" given via snippet array entry "scss" -->' . "\n  ";
  };
};
if(!isset($template)) {
  $template = $page->template();
  if($Echo4Developer) {
    echo '<!-- scss file "' . $template . '.critical.scss" selected by snippet "scss" for template = "' . $template . '.php" -->' . "\n  ";
  };
};

// Set file paths. Used for checking and updating CSS file for current template.
$SCSS            = $root . '/assets/scss/' . $template . '.critical.scss';
$minCSS          = $root . '/assets/css/'  . $template . '.critical.min.css';
$devCSS          = $root . '/assets/css/'  . $template . '.critical.dev.css';
$minCSSKirbyPath = 'assets/css/' . $template . '.critical.min.css';
$devCSSKirbyPath = 'assets/css/' . $template . '.critical.dev.css';
...
if(($template <> 'default') AND !file_exists($SCSS)) {
  $SCSS            = $root . '/assets/scss/default.critical.scss';
  $minCSS          = $root . '/assets/css/default.critical.min.css';
  $devCSS          = $root . '/assets/css/default.critical.dev.css';
  $minCSSKirbyPath = 'assets/css/default.critical.min.css';
  $devCSSKirbyPath = 'assets/css/default.critical.dev.css';
  if($Echo4Developer) {
    echo '<!-- file "/assets/scss/' . $template . '.critical.scss" NOT found => use "default.scss"! Ok! -->' . "\n  ";
  };
} else {
  if($Echo4Developer) {
    echo '<!-- file "/assets/scss/' . $template . '.critical.scss" found => use "' . $template . '.critical.scss", NOT "default.critical.scss" -->' . "\n  ";
  };
};
